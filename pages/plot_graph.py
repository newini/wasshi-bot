...
def plot_graph():
    ...
    graph = dict(
        data=[
            dict(
                ...
                textfont=dict(family="sans serif", size=24, color="#ff7f0e"),
            )
        ]
    )
    layout = dict(
        ...
        xaxis=dict(
            title="Date", nticks=4, titlefont=dict(size=18), tickfont=dict(size=18)
        ),
        yaxis=dict(
            ticksuffix="°C",
            title="Temperature",
    ...
    figure.update_xaxes(
        range=[
            datetime.datetime.strptime(x[0], "%Y-%m-%d %H:%M:%S")
            - datetime.timedelta(hours=4),
            datetime.datetime.strptime(x[-1], "%Y-%m-%d %H:%M:%S")
    ...
    return render_template(
        "plot_graph.html", graph_json=graph_json, layout_json=layout_json
    )
